...
class OcpFesBuilder:
    ...
    def __init__(self):
        ...
        self.time_min = None
        self.time_max = None
        self.time_bimapping = False
        self.pulse_duration = None
        self.pulse_duration_min = None
        self.pulse_duration_max = None
        self.pulse_duration_bimapping = False
        self.pulse_intensity = None
        self.pulse_intensity_min = None
        self.pulse_intensity_max = None
        self.pulse_intensity_bimapping = False
        self.force_tracking = None
        self.end_node_tracking = None
        self.custom_objective = None
        self.use_sx = True
        self.ode_solver = OdeSolver.RK4(n_integration_steps=1)
        self.n_threads = 1
    ...
    def set_n_stim_from_frequency_and_final_time(self, frequency, final_time, round_down):
        self.n_stim = self.from_frequency_and_final_time(frequency, final_time, round_down)
        return self

    def set_n_stim_from_frequency_and_n_stim(self, frequency, final_time, round_down):
        self.n_stim = self.from_frequency_and_n_stim(frequency, final_time, round_down)
        return self

    def set_n_shooting_per_phae(self, n_shooting):
    ...
    def set_fes_parameters(self, parameters: dict[str, tuple[float, float]]):

        # check in function the model set before.

        # parameters = {"time" : (time_min, time_max),
        # "pulse_duration" : (pulse_duration_min, pulse_duration_max),
        # "pulse_intensity" : (pulse_intensity_min, pulse_intensity_max)
        # }


        self.fes_parameters = parameters

    def set_parameter_equality_constraint(self, key: str):
        pass

    def set_force_tracking(self, temps, serie):
        #  merge end_node tracking
        if not isinstance(force_tracking, list):
            raise TypeError("force_tracking must be list type")
        if not all(isinstance(x, np.ndarray) for x in force_tracking):
    ...
    def build(self):
        return return OptimalControlProgram(
            bio_model=models,
            dynamics=dynamics,
